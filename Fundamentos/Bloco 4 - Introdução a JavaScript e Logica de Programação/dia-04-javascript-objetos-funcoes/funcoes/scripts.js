...
// FUNCTION PROGRAMA PARA CALCULAR SALARIO LIQUIDO DE ACORDO COM DESCONTOS DOS IMPOSTOS.

function salarioDescontadoINSS (){
...
function calculoIR () {
  ...
  let salarioDescontoINSS = salarioDescontadoINSS();

  if (salarioDescontoINSS <= 1903.98) {
    aliquotaIR = 0;
  }
  else if (salarioDescontoINSS <= 2826.65) {
    aliquotaIR = (salarioDescontoINSS * 0.075) - 142.80;
  }
  else if (salarioDescontoINSS <= 3751.05) {
    aliquotaIR = (salarioDescontoINSS * 0.15) - 354,80;
  }
  else if (salarioDescontoINSS <= 4664.68) {
    aliquotaIR = (salarioDescontoINSS * 0.225) - 636.13;
  } 
  else {
    aliquotaIR = (salarioDescontoINSS * 0.275) - 869.36;
  }
  return aliquotaIR;
}

let salarioLiquido = salarioDescontadoINSS () - calculoIR ();
